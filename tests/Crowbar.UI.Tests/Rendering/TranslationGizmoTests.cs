...
public class TranslationGizmoTests
{
    ...
    public void Hover_DetectsTheAxisUnderTheMouse()
    {
        ...
        var (ray, pixels) = RayThroughPoint(new Vector3(0.8f, 0f, 0f), camera);

        gizmo.UpdateHover(ray, pixels, camera.ViewMatrix, camera.ProjectionMatrix(Aspect), Width, Height);

        Assert.Equal(TranslationGizmo.Axis.X, gizmo.HoveredAxis);
    }
    ...
    public void Hover_IgnoresPointsBeyondTheShaft()
    {
        ...
        var camera = new Camera();
        // On the X axis line but far beyond the shaft (t = 5 > AxisLength).
        var (ray, pixels) = RayThroughPoint(new Vector3(5f, 0f, 0f), camera);

        gizmo.UpdateHover(ray, pixels, camera.ViewMatrix, camera.ProjectionMatrix(Aspect), Width, Height);

        Assert.Equal(TranslationGizmo.Axis.None, gizmo.HoveredAxis);
    }
    ...
    public void Drag_MovesTheTargetAlongTheActiveAxis()
    {
        ...
        var (startRay, startPixels) = RayThroughPoint(new Vector3(0.6f, 0f, 0f), camera);
        gizmo.UpdateHover(startRay, startPixels, view, projection, Width, Height);
        Assert.Equal(TranslationGizmo.Axis.X, gizmo.HoveredAxis);
        gizmo.BeginDrag(startRay);

        var (dragRay, _) = RayThroughPoint(new Vector3(1.4f, 0f, 0f), camera);
        gizmo.Drag(dragRay);
    ...
    public void Drag_SnapsTheMovementToTheSnapSize()
    {
        ...
        var (dragRay, _) = RayThroughPoint(new Vector3(1.2f, 0f, 0f), camera);
        gizmo.Drag(dragRay);
    ...
    public void LocalAxes_FollowTheTargetRotation()
    {
        ...
        var gizmo = new TranslationGizmo();
        gizmo.SetTarget(transform);
        AssertEqual(gizmo.AxisDirection(TranslationGizmo.Axis.X), 1f, 0f, 0f);

        // With local axes, a 90 degree yaw maps +X to -Z and +Z to +X.
        gizmo.LocalAxes = true;
        gizmo.SetTarget(transform);
        AssertEqual(gizmo.AxisDirection(TranslationGizmo.Axis.X), 0f, 0f, -1f);
        AssertEqual(gizmo.AxisDirection(TranslationGizmo.Axis.Z), 1f, 0f, 0f);
    }
    ...
    public void EndDrag_ReleasesTheActiveAxis()
    {
        ...
        var view = camera.ViewMatrix;
        var projection = camera.ProjectionMatrix(Aspect);
        var (ray, pixels) = RayThroughPoint(new Vector3(0.6f, 0f, 0f), camera);
        ...
        Assert.Equal(TranslationGizmo.Axis.None, gizmo.ActiveAxis);
    }
