...
public abstract class Gizmo
{
    ...
    /// can never drift apart: depth comes from the view matrix (third row is
    /// -forward for a row-vector look-at), FOV from the projection
    /// (M22 = 1 / tan(fov/2)).
    /// </summary>
    public static float ScreenConstantWorldSize(Vector3 origin, Matrix4x4 view, Matrix4x4 projection, int height, float pixels)
    {
        var forwardRow = new Vector3(view.M31, view.M32, view.M33);
        var depth = Math.Max(1e-4f, -(Vector3.Dot(forwardRow, origin) + view.M43));
